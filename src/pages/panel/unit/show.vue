<template>
  ...
  <q-tab-panels v-model="tab" animated class="bg-transparent">
          ...
          <q-banner>
            <div class="row q-col-gutter-md items-center">
              <div class="col-12 flex justify-center">
                <q-btn-group>
                  <q-btn color="secondary" label="ثبت تراکنش" icon="payments" :to="{ name: 'Panel.Unit.Transaction.Create', params: { id: unitId } }" />
                  <q-btn color="secondary" label="ویرایش اطلاعات" icon="edit" :to="{ name: 'Panel.Unit.Edit', params: { id: unitId } }" />
                </q-btn-group>
              </div>
              <div class="col-12">
        ...
        <q-card-section>
          <div class="row q-col-gutter-md">
            <div class="col-md-6 col-12">
              <unit-user-list :unit-id="unitId" :users="unitData.residents" user-role="resident" :edit-mode="false" />
            </div>
            <div class="col-md-6 col-12">
              <unit-user-list :unit-id="unitId" :users="unitData.owners" user-role="owner" :edit-mode="false" />
            </div>
          </div>
        </q-card-section>
      </q-card>
        ...
        <span :class="{'text-green': inputData.props.row.invoice?.is_covered_by_monthly_charge, 'text-red': !inputData.props.row.invoice?.is_covered_by_monthly_charge}">
          ...
          <template v-else-if="inputData.col.name === 'actions'">
            <div class="action-column-entity-index">
              ...
              />
              <!--          <q-btn v-if="inputData.props.row.status !== 'paid'"-->
              <!--                 color="primary"-->
              <!--                 flat-->
              <!--                 icon="payment"-->
              <!--                 :to="{ name: 'Panel.InvoiceDistribution.Pay', params: { invoice_distribution_id: inputData.props.row.id, unit_id: inputData.props.row.unit.id } }"-->
              <!--          />-->
            </div>
          </template>
          <template v-else>
    ...
    <q-tab-panel name="monthlyReport">
      <entity-index :value="invoiceIndexInputs"
    ...
    <q-tab-panel name="transactions">
      <entity-index
        :value="transactionIndexInputs"
        :title="transactionIndexLabel"
        :api="transactionIndexApi"
        :table="transactionIndexTable"
        :table-keys="transactionIndexTableKeys"
        :show-route-name="transactionShowRouteName"
        :show-create="false"
        :show-close-button="false"
        :show-expand-button="false"
        :show-reload-button="false"
        :show-search-button="true"
        :row-key="transactionIndexItemIdentifyKey"
      >
        <template #entity-index-table-cell="{ inputData }">
          <template v-if="inputData.col.name === 'amount'">
            <currency-number :number="inputData.props.row.amount" />
          ...
          <template v-else-if="inputData.col.name === 'actions'">
            <div class="action-column-entity-index">
              <q-btn
                ...
                flat
                icon="visibility"
                :to="{ name: transactionIndexShowRouteName, params: { id: inputData.props.row.id } }"
              />
            </div>
          </template>
          <template v-else>
...
import type { TransactionType } from 'src/repositories/transaction';
import UnitProfileTabHeader from 'src/components/UnitProfileTabHeader.vue';
import TransactionAPI, { transactionTargetGroupOptions } from 'src/repositories/transaction';
...
const transactionShowRouteName = ref('Panel.Transaction.Payment.Show');
...
const transactionIndexApi = ref(transactionAPI.endpoints.base);
const transactionIndexLabel = ref('پرداخت های واحد');
const transactionIndexShowRouteName = ref('Panel.Transaction.Payment.Show');
const transactionIndexItemIdentifyKey = ref('id');
const transactionIndexTableKeys = ref({
  data: 'data',
  total: 'total',
  currentPage: 'current_page',
  perPage: 'per_page',
...
const transactionIndexTable = ref({
  columns: [
    {
      name: 'amount',
      required: true,
...
const transactionIndexInputs = ref([
  {
    type: 'hidden',
    name: 'sortation_field',
    value: 'created_at'
